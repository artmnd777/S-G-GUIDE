...
function clientReplyPrompt() {
  return [
    'Режим: Ответ клиенту.',
    '',
    'Я напишу готовый красивый текст, который можно сразу отправить клиенту. Живым языком, коротко и по делу.',
    '',
    'Опишите ситуацию.',
    '',
    'Например:',
...
bot.on('text', async (ctx) => {
  ...
  if (userText === 'Начать сначала') {
    const currentAction = getPendingAction(ctx);
    histories.delete(ctx.chat.id);
    if (currentAction === 'client_reply') {
      setPendingAction(ctx, 'client_reply');
      await ctx.reply('Готово. Старый контекст очищен. Опишите новую ситуацию, и я напишу готовый ответ клиенту.', clientReplyMenu());
      return;
    }
  ...
  if (userText === 'Ответ клиенту') {
    setPendingAction(ctx, 'client_reply');
    await ctx.reply(clientReplyPrompt(), clientReplyMenu());
    return;
  }
  ...
  if (userText === 'Правило поведения') {
    ...
    await ctx.reply('Отправьте правило поведения. Например: отвечай короче и сначала давай готовый текст клиенту.', mainMenu());
    return;
  }
  ...
  if (pendingAction) {
    ...
    if (['analyze_client', 'analyze_situation', 'client_reply', 'objection_work', 'manager_training', 'casual_chat'].includes(pendingAction)) {
      ...
      const prefixes = {
        ...
        client_reply: 'Напиши готовый красивый ответ клиенту живым языком, коротко и по делу. Ситуация:',
